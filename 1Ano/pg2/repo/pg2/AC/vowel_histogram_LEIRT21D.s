...
main:
	...
	mov		r1, #SIZE * 2
	bl 		fill_zero			; fill_zero(occurrence1, SIZE * 2);
	ldr		r0, occurrences2_addr
	mov		r1, #SIZE * 2
	bl 		fill_zero			; fill_zero(occurrence2, SIZE * 2);
	ldr		r0, phrase1_addr
	mov		r1, #7
...
vowel_histogram_for_body:
	...
	lsl		r0, r0, #1
	ldr		r1, [r6, r0]		; r1 = occurrences[idx]
...
fill_zero:
	mov		r2, #0 			; i = 0
	mov		r3, # 0			; valor zero
	b 		fill_zero_for_ctrl
fill_zero_for_body:
	strb	r3, [r0, r2]	; a[i] = 0
	add		r2, r2, #1		; i++
fill_zero_for_ctrl:		
	cmp		r2, r1			; i < size
	blo		fill_zero_for_body
	mov		pc, lr
